...
final class DictationManager: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.gmalonso.whisper-dictate-mac", category: "DictationManager")
    ...
    private func updateCumulativeStats(wordCount: Int, audioDuration: TimeInterval) {
        let totalDictations = defaults.integer(forKey: MacAppKeys.syncTotalDictations)
        defaults.set(totalDictations + 1, forKey: MacAppKeys.syncTotalDictations)

        let totalWords = defaults.integer(forKey: MacAppKeys.syncTotalWords)
        defaults.set(totalWords + wordCount, forKey: MacAppKeys.syncTotalWords)

        let totalDuration = defaults.double(forKey: MacAppKeys.syncTotalDurationSeconds)
        defaults.set(totalDuration + audioDuration, forKey: MacAppKeys.syncTotalDurationSeconds)
    }
