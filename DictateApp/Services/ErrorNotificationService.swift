...
final class ErrorNotificationService: NSObject, UNUserNotificationCenterDelegate {
    ...
    func showRateLimitError() {
        showError(
            title: String(localized: "Limite de Uso Atingido"),
            message: String(localized: "Muitas requisições. Aguarde um momento antes de tentar novamente.")
        )
    }

    func showWeeklyCapError() {
        showError(
            title: String(localized: "Limite Semanal Atingido"),
            message: String(localized: "Você atingiu o limite semanal de transcrições. Faça upgrade do seu plano para mais.")
        )
    }

    func showNetworkError() {
        showError(
            title: String(localized: "Sem Conexão"),
            message: String(localized: "Verifique sua conexão com a internet e tente novamente.")
        )
    }

    func showServerError() {
        showError(
            title: String(localized: "Erro no Servidor"),
            message: String(localized: "Algo deu errado do nosso lado. Tente novamente mais tarde.")
        )
    }
