...
private let logger = Logger(subsystem: "com.gmalonso.whisper-dictate-mac", category: "Pipeline")
...
enum TranscriptionPipeline {
    ...
    static func transcribe(
        ...
        logger.info(
            ...
            dictionaryPreview=\(previewDictionaryTerms(dictionaryTerms), privacy: .public)
            """
        )
        ...
        logger.info("Final text after post-process=\(loggableText(finalText), privacy: .public)")
...
private extension TranscriptionPipeline {
    ...
    static func postProcess(
        ...
        if afterDictionary != route.text {
            logger.info(
                """
                Dictionary adjusted text: before=\(loggableText(route.text), privacy: .public), \
                after=\(loggableText(afterDictionary), privacy: .public)
                """
            )
        } else {
...
private extension TranscriptionPipeline {
    static func loggableText(_ text: String?) -> String {
        guard let text else { return "nil" }
        let flattened = text.replacingOccurrences(of: "\n", with: "\\n")
        if flattened.count <= 300 {
            return flattened
        }
        return String(flattened.prefix(300)) + "..."
    }

    static func previewDictionaryTerms(_ terms: [String]) -> String {
        guard !terms.isEmpty else { return "[]" }
        let preview = terms.prefix(12).joined(separator: ", ")
        return terms.count > 12 ? "[\(preview), ...]" : "[\(preview)]"
    }
}
