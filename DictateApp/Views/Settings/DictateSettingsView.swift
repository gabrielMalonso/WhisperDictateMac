...
struct DictateSettingsView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                SettingsComponents.brandedHeader("dictate")
        ...
        .onChange(of: translationTargetLanguage) { _, _ in syncSettings() }
        .task {
            clearDeprecatedTranscriptionDomainIfNeeded()
            sanitizeProLockedState()
        }
    }
            ...
            SettingsComponents.sectionHeader(String(localized: "Transcrição"))
                ...
                HStack(spacing: SettingsComponents.rowSpacing) {
                    ...
                    .onChange(of: formattingOptions.tone) { _, newValue in
                        if FeatureGate.canUseTone(newValue) {
        ...
        SettingsComponents.card {
            SettingsComponents.sectionHeader(String(localized: "Tradução"))
                HStack(spacing: 12) {
                    ...
                    if FeatureGate.canUseTranslation {
                ...
                if FeatureGate.canUseTranslation && translationEnabled {
                    SettingsComponents.divider()
    ...
    private func sanitizeTranslationState() {
        if translationEnabled, !FeatureGate.canUseTranslation {
            translationEnabled = false
        }
    }

    private func sanitizeToneState() {
        guard !FeatureGate.canUseTone(formattingOptions.tone) else { return }
        formattingOptions.tone = .natural
        formattingOptions.save()
    }

    private func sanitizeProLockedState() {
